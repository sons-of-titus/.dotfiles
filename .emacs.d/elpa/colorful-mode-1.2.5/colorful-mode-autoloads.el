...
With prefix ARG, enable Global Colorful mode if ARG is positive; otherwise, disable it.

If called from Lisp, toggle the mode if ARG is `toggle'.
Enable the mode if ARG is nil, omitted, or is a positive number.
Disable the mode if ARG is a negative number.

Colorful mode is enabled in all buffers where `turn-on-colorful-mode' would do it.

See `colorful-mode' for more information on Colorful mode.

`global-colorful-modes' is used to control which modes this minor mode is used in.
